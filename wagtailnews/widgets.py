...
class AdminNewsChooser(BaseChooser):
    class Media:
    ...
    def render_html(self, name, value, attrs):
        ...
        original_field_html = super(AdminNewsChooser, self).render_html(
            name, value, attrs
        )

        return render_to_string(
            "wagtailnews/widgets/news_chooser.html",
            {
                "widget": self,
    ...
    def render_js_init(self, id_, name, value):
        return "createNewsChooser({id}, {type});".format(
            id=json.dumps(id_), type=json.dumps(str(self.target_model._meta))
        )
